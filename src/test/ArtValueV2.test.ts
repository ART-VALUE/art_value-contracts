...
import { AllEvents, ArtValueV2Instance, Transfer } from "@contract/ArtValueV2";
...
contract("ArtValueV2", accounts => {
  ...
  it('has a name', async function () {
    const avc = this.avc as ArtValueV2Instance
  ...
  it('allows minters to mint', async function () {
    ...
    const tokenOwner = await avc.ownerOf(1)
    expect(tokenOwner).to.equal(tokenReceiver)
  });
  ...
  describe('handles bulk minting', () => {

    it('allows bulk minting with number', async function () {
      const avc = this.avc as ArtValueV2Instance
      const numbers = [new BN(8801), new BN(8802), new BN(8803), new BN(8804)]
  
      const balanceBefore = await avc.balanceOf(tokenReceiver)
      expect(balanceBefore.toNumber()).to.equal(0)
      await avc.mintWithNumberBulk(
        tokenReceiver,
        numbers,
        { from: creator }
      )
      ...
      for (let i = 0; i < numbers.length; i++) {
        const expectedNumber = numbers[i];
        const tokenId = i + 1;
        const actualNumber = await avc.numberByToken(tokenId)
        expect(actualNumber.eq(expectedNumber)).to.be.true
        const tokenOwner = await avc.ownerOf(tokenId)
        expect(tokenOwner).to.equal(tokenReceiver)
      }
    });

    it('allows bulk minting with placeholder', async function () {
      const avc = this.avc as ArtValueV2Instance
      const nbroTokensToMint = new BN(10)
  
      const balanceBefore = await avc.balanceOf(tokenReceiver)
      expect(balanceBefore.toNumber()).to.equal(0)
      await avc.mintAsPlaceholderBulk(
        tokenReceiver,
        nbroTokensToMint,
        { from: creator }
      )
      const balanceAfter = await avc.balanceOf(tokenReceiver)
      expect(balanceAfter.toNumber()).to.equal(nbroTokensToMint.toNumber())
    });
